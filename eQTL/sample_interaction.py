...
inter_sig_right_anchor_XOR = inter_sig_right_anchor_XOR[['chr1','start1','end1','chr2','start2','end2','count','expected','fdr','ClusterLabel','ClusterSize',
                                                         'ClusterType','ClusterNegLog10P','ClusterSummit','sig_ID','chr','start','end','anchor_ID']]

inter_sig_XOR_reformed = pd.concat([inter_sig_left_anchor_XOR, inter_sig_right_anchor_XOR])
inter_sig_XOR_reformed = inter_sig_XOR_reformed[['chr1','start1','end1','chr2','start2','end2','count','expected','fdr','ClusterLabel','ClusterSize',
                                                         'ClusterType','ClusterNegLog10P','ClusterSummit','sig_ID','anchor_ID']]

inter_sig_XOR_reformed["dist_sig"] = abs(inter_sig_XOR_reformed["start2"] - inter_sig_XOR_reformed["start1"])
...
print (inter_sig_XOR_reformed["dist_sig"].describe())
...
inter_sig_XOR_reformed_binned = inter_sig_XOR_reformed.groupby(pd.cut(inter_sig_XOR_reformed["dist_sig"].abs(), distance_bins))
print ("distance distribution (sig interaction): ")
print(inter_sig_XOR_reformed_binned.count()["dist_sig"])

sample_size_mdist = [i*int(power) for i in inter_sig_XOR_reformed_binned.count()["dist_sig"].tolist()]
#%%
...
inter_all_reformed_sig = pd.merge(inter_all_reformed, inter_sig_XOR_reformed, how="inner",
...
print ("duplicates:", inter_all_reformed_nonsig[["ID"]].shape[0] - inter_all_reformed_nonsig[["ID"]].drop_duplicates().shape[0])
...
inter_all_reformed_nonsig_binned = inter_all_reformed_nonsig.groupby(pd.cut(inter_all_reformed_nonsig["dist"], distance_bins))
print ("distance distribution before sampling (all pairs): ")
print (inter_all_reformed_nonsig_binned.count()["dist"])

#%%
inter_all_reformed_nonsig_binned_df_list = [group.reset_index().drop(["index"], axis=1) for _, group in inter_all_reformed_nonsig_binned]    
inter_all_reformed_nonsig_sampled_list = [inter_all_reformed_nonsig_binned_df_list[i].sample(n=sample_size_mdist[i]) for i in range(len(sample_size_mdist))]                    
inter_all_reformed_nonsig_sampled_concat = pd.concat(inter_all_reformed_nonsig_sampled_list).reset_index().drop(["index"], axis=1)
inter_all_reformed_nonsig_sampled_binned = inter_all_reformed_nonsig_sampled_concat.groupby(pd.cut(inter_all_reformed_nonsig_sampled_concat["dist"], distance_bins))
print ("distance distribution after sampling (all pairs): ")
print(inter_all_reformed_nonsig_sampled_binned.count()["dist"])
...
sampled_nonsig_anchor = inter_all_reformed_nonsig_sampled_concat[['chr1','start1','end1','fdr','ID', "dist"]]
sampled_nonsig_anchor.to_csv(r'C:\Users\libin\UCSF\hfb\eqtl\20200115\interaction_sampled_nonsig_anchor_{}_{}'.format(cellType, iteration), sep="\t", index=False, header=False)
sampled_nonsig_target = inter_all_reformed_nonsig_sampled_concat[['chr2','start2','end2','fdr','ID', "dist"]]
sampled_nonsig_target.to_csv(r'C:\Users\libin\UCSF\hfb\eqtl\20200115\interaction_sampled_nonsig_target_{}_{}'.format(cellType, iteration), sep="\t", index=False, header=False)

inter_all_reformed_nonsig_sampled_concat.to_csv(r'C:\Users\libin\UCSF\hfb\eqtl\20200115\interaction_sampled_nonsig_{}_{}'.format(cellType, iteration), sep="\t", index=False, header=False)
